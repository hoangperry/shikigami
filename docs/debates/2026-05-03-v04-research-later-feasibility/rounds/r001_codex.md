...
session id: 019de937-8628-7571-b285-18b265c104f4
--------
user
...
You are a participant in a four-way AI debate auditing whether 6 GitHub issues are TRULY infeasible to implement, or whether Claude (Opus) was too quick to defer them.

Project: Shikigami — Tauri 2 + Rust + React desktop overlay app showing animated 2D character reacting to AI coding events. macOS primary, Windows + Linux scaffolding shipped (CI builds bundles, runtime untested on hardware). Codex CLI hook bridge just shipped (95% reuse of Claude Code path).

Dev environment: macOS only, no Windows or Linux hardware available, no $300-400/yr EV cert budget. All implementation done by Claude (Opus) via CLI tools.

Issues moved to "v0.4 Research Later" milestone:
...
YOUR TASK (Round 1, ~400 words):

For each of the 6 issues (#29, #30, #31, #33, #34, #35), give a verdict:
- **CLOSE-WITH-DOC** if truly infeasible from a Mac with no extra budget/hardware; the research doc IS the resolution
- **KEEP-OPEN** if blocked-but-worth-tracking; revisit later
- **DO-NOW** if Claude missed a creative path that's actually shippable today

For each verdict, give ONE sentence of justification. Then add ONE paragraph at the end identifying which issue Claude is most likely WRONG about (i.e., where you suspect the "infeasible" label is overcautious).

Be opinionated. Engineering time is the user's priority — they want to know what to NOT spend time on, and what was wrongly deferred.
codex
#29: **KEEP-OPEN** — Mac-only dev cannot validate Windows compositor, DPI, fullscreen, or click-through behavior honestly, but the shipped CI scaffolding makes this a real platform-readiness tracker rather than something to close.

#30: **CLOSE-WITH-DOC** — Without EV cert budget or a borrowed signing setup, there is no meaningful implementation path beyond documenting unsigned alpha builds and the future CI signing steps.

#31: **KEEP-OPEN** — Linux overlay behavior is compositor-dependent enough that macOS cannot prove it, but this should remain a tracked validation gap for v0.3/v0.4 instead of being treated as resolved research.

#33: **DO-NOW** — Cursor hooks are documented and structurally close to the existing Claude/Codex hook bridge, so the sensible path is to implement a conservative adapter for confirmed events, ignore unknowns, and document limitations instead of waiting for perfect survey certainty.

#34: **KEEP-OPEN** — Windsurf’s existence is confirmed but payload shape is not, so implementation should wait for captured real payloads; however, the pre-work itself is small and should remain tracked.

#35: **KEEP-OPEN** — A VSCode extension bridge is feasible, but building on a Preview hooks API risks churn, so it is worth tracking but not worth spending engineering time on before API stabilization.

Claude is most likely wrong about **#33 Cursor**. This is the one where “research later” looks overcautious rather than genuinely blocked: the docs exist, the app already has a working hook ingestion path, and schema uncertainty can be handled with a tolerant transformer plus fixture-driven logging. It does not need hardware, paid certificates, or upstream GA status. The highest-value move is probably a minimal Cursor bridge that supports the common lifecycle events now and records unmapped payloads for later refinement.
tokens used
16,641
#29: **KEEP-OPEN** — Mac-only dev cannot validate Windows compositor, DPI, fullscreen, or click-through behavior honestly, but the shipped CI scaffolding makes this a real platform-readiness tracker rather than something to close.
